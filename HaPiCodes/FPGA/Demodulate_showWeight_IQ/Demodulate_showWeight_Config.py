...
def config_FPGA_registers(module_: SD1.SD_Module, channel:int, registers: FPGARegisters, module_name: str = None):
    ...
    for k, v in registers.__dict__.items():
        if k == "wf_trig_delay":
            ...
            warn_msg = f"weight function trigger delay {v} is smaller than {min_eff_} or wf_data_file is null, " \
...
def configFPGA(module: SD1.SD_Module, channel:int,
               demod_trunc: int,  wf_data: List[List] = None, wf_start: int = None, wf_stop:int = None,
    ...
    write_FPGA_memory(module, f"WeightFunc_{channel}_I", np.array(wf_data)[:, 0])
    write_FPGA_memory(module, f"WeightFunc_{channel}_Q", np.array(wf_data)[:, 1])