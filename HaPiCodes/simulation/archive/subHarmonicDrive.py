...
H0 = (omega_a - 2 * alpha) * a.dag() * a + alpha / 6 * (a.dag() + a) ** 4
...
if __name__ == "__main__":
    o_d_list = (omega_d + np.linspace(-0.045, -0.042, 31) * 2 * PI)
    time_list = np.linspace(20, 520, 26)
    exp_n_list = np.zeros((len(o_d_list), len(time_list), dim))
    for i, o_d in enumerate(o_d_list):
        print(i)
        sweepDriveTime = partial(sweepDriveFreqAndTime, o_d)
        exp_n = parallel_map(sweepDriveTime, time_list, progress_bar=True)
        exp_n_list[i] = np.array(exp_n)
        saveData(r"./SimData/SubHarmonicDrive_NonRWA_SmoothDrive_SweepFreqTime//",
                 f"DriveFactor_5_DriveFreq_{i}", DriveFreq=o_d, data=exp_n,
                 time_list=time_list, dim=dim)

    plt.figure()
    plt.pcolormesh((o_d_list - omega_d) / 2 / PI, time_list, exp_n_list[:, :, 1].T)
    plt.colorbar()

    # linecut------------------------------------
    # plt.figure()
    # drive_freq_idx = np.argmin(np.abs((o_d_list-omega_d)/2/PI+0.0345))
    # for i in range(dim):
    # plt.plot(time_list, exp_n_list[0,:, i])
