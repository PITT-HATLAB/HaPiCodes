...
if __name__ == "__main__":
    ...
    time_list = np.linspace(50, 150, 51)
    omegaAndTime = np.array([[[i, j] for i in o_d_list] for j in time_list]).flatten().reshape(-1, 2)
    exp_n_list = np.zeros((len(o_d_list), len(time_list), dim))
    exp = parallel_map(sweepDriveFreqAndTime2D, omegaAndTime, progress_bar=True)
    saveData(r"./SimData//",
                 f"DriveFactor_12_50Ramp_with6Order_frequencyShift", data=exp,
                 time_list=time_list, dim=dim, o_d_list=o_d_list)

    res2D = np.array(exp).reshape(len(time_list), len(o_d_list), dim)
    plt.figure()
    plt.pcolormesh((o_d_list - omega_d) / 2 / PI, time_list, res2D[:, :, 1], shading='auto')
    plt.colorbar()
    #
    # # linecut------------------------------------
    # plt.figure()
