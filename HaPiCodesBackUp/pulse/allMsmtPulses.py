...
def driveAndMsmt(module_dict):
    ...
    W = pc.modulesWaveformCollection(module_dict)
    gPulse = pc.gau(gauCondition)
    bPulse = pc.box(boxCondition)
    box50 = pc.box(box50MHzCondition)
    ...
    W.A2['pulse.msmtBox50.I'] = 0.3 * np.sin(2 * np.pi * xdata / 20)#box50.smooth().I_data
    W.A2['pulse.test'] = box50.smooth().I_data
    ...
    # define Queue here
    Q = pc.modulesQueueCollection(module_dict)
    for i in range(1):
        Q.addTwoChan('A1', [1, 2], i, ['pulse.gau.I', 'pulse.gau.Q'], 200)
        Q.addTwoChan('A1', [3, 4], i, ['pulse.msmtBox.I', 'pulse.msmtBox.Q'], 500)
        Q.add('A2', 1, i, 'pulse.msmtBox50.I', 500)
        Q.add('A2', 1, i, 'pulse.test', 2500)

        Q.add('M1', 1, i, 'pulse.gau.M', 100)
        Q.add('M1', 2, i, 'pulse.msmtBox.M', 400)
        Q.add('D1', 1, i, 'trigger.dig', 0)
        Q.add('D1', 2, i, 'trigger.dig', 0)
        Q.add('D1', 3, i, 'trigger.dig', 0)
        Q.add('D1', 4, i, 'trigger.dig', 0)
        Q.add('D1', 1, i, 'trigger.fpga4', 20)
        Q.add('D1', 1, i, 'trigger.fpga5', 20)
        Q.add('D1', 1, i, 'trigger.fpga6', 20)
        Q.add('D1', 1, i, 'trigger.fpga7', 20)
...
def pulseSpecReal(module_dict):
    ...
    # define all waveforms we're going to use into W
    W = pc.modulesWaveformCollection(module_dict)
    qPulse = pc.box(qubitBoxCondition)
    bPulse = pc.box(boxCondition)
...
def t1MsmtReal(module_dict, timeArray=np.linspace(0, 300000, 101)[:100]):
    ...
    for i in range(len(timeArray)):
        ...
        Q.add('D1', 2, i, 'trigger.fpga5', timeIndex + 20)
        # TODO: It seems that digitizer will only do one thing, send trigger, maybe we should just use a common name and translate
        # TODO: that to FPGA4-7, or DAQ1-4 based on the channel and firmware used
