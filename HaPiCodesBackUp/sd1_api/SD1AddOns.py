...
class AIN(SD1.SD_AIN):
    ...
    def FPGAload(self, FPGA_file):
        try:            
            ...
            warnings.warn(str(e) + " will take the FPGA_file as path to a user generated FPGA")
    ...
    def DAQAutoConfig(self, trig_nums:dict, avg_num:int, max_trig_num_per_exp: int  = None,
                      msmt_length_list: Optional[List[int]] = None, triggerMode = SD1.SD_TriggerModes.SWHVITRIG):
        """
        ...
        :param msmt_length_list: List of msmt length for each channel, in ns. Should only be provided when subbuffer
            is not used
        ...
        if not self.subbuffer_used:
            if  msmt_length_list is None:
                raise ValueError("Looks like you are using the demodulate FPGA, "
                                 "in this case, msmt_length_list must be provided")
            else:
                ppc_list = np.array(msmt_length_list)//10*5
                cyc_list = dig_trig_num_list * avg_num
                avg_num_per_hvi = avg_num
                self.avg_num = avg_num

        else:
            if msmt_length_list is not None:
                warnings.warn("msmt_length_list will be omitted for the auto configuration of DAQs with subbuffer used")
            cycles = np.ceil(max_trig_num / self.FPGA_markup["subbuffer_size"])
    ...
    def DAQreadArray(self, channel, timeOut = 10, reshapeMode = "nAvg"):
        ...
        :param reshapeMode: "nCycyles" or "nAvg"

        :return: for demodulate FPGA (no subbuffer used) the return is np float array
                    with shape (nCycles, pointsPerCycle) or (avg_num, msmt_num_per_exp, msmt_length//10)
