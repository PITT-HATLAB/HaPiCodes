...
import time
from AddOns.SD1AddOns import AIN, AOU
# from keysightSD1 import SD_AIN as AIN
# from keysightSD1 import SD_AOU as AOU
...
preOffset = {'A1': [-0.01186, 0.00091, 0.0454, 0.018],
               "A2": [0, 0, 0, 0],
               "A3": [0, 0, 0, 0],
               "M1": [0, 0, 0, 0],
               "M2": [0, 0, 0, 0],
    ...
    for module in module_dict:
        if module_dict[module].instrument.getProductName() != "M3102A":
            ...
            for i in range(1, 5):
                ...
                module_dict[module].instrument.channelInputConfig(i, 0.0625, impedance, coupling)
...
def define_instruction_compile_hvi(module_dict: dict, Q, pulse_general_dict: dict, subbuffer_used=0):
    ...
    for seqOrder in range(Q.maxIndexNum + 1):  # the maxIndexNum should be the len(xdata)
        delay = int(pulse_general_dict['relaxingTime'] * 1e3) + 30 if seqOrder == 0 else int(pulse_general_dict['relaxingTime'] * 1e3) - timeMax
    ...
    syncBlock = syncWhile.sync_sequence.add_sync_multi_sequence_block(f"syncBlockSub", delay)
    ...
    regIncInstru = seq.add_instruction('regIncreaseSubBuffer', 200, seq.instruction_set.add.id)
    ...
    if subbuffer_used:
        lastBlock = sequencer.sync_sequence.add_sync_multi_sequence_block(f"readSub", 5000)
